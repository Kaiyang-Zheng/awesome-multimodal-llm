...
如果每一项 $q_i k_i$ 的方差大致固定，并且这些项彼此独立，那么根据独立随机变量求和时方差可加，有：
...
$$
\mathrm{Var}(q_i k_i) = \mathbb{E}[(q_i k_i)^2] - (\mathbb{E}[q_i k_i])^2
$$

由于 $\mathbb{E}[q_i k_i] = 0$，可得：

$$
\mathrm{Var}(q_i k_i) = \mathbb{E}[q_i^2 k_i^2]
$$
...
$$
\mathbb{E}[q_i^2] = \mathrm{Var}(q_i) = 1, \qquad \mathbb{E}[k_i^2] = \mathrm{Var}(k_i) = 1
$$
...
$$
\mathrm{Var}(q^\top k) = \mathrm{Var}\left(\sum_{i=1}^{d_k} q_i k_i\right) = \sum_{i=1}^{d_k} \mathrm{Var}(q_i k_i)
$$

每一项方差都为 1，因此：

$$
\mathrm{Var}(q^\top k) = \sum_{i=1}^{d_k} 1 = d_k
$$
...
$$
\mathrm{Var}(q_i) = \sigma_q^2, \qquad \mathrm{Var}(k_i) = \sigma_k^2
$$
...
$$
\mathrm{Var}(q^\top k) = d_k \sigma_q^2 \sigma_k^2
$$
...
$$
\mathrm{Var}\left(\frac{q^\top k}{\sqrt{d_k}}\right) = \frac{1}{d_k}\mathrm{Var}(q^\top k) = \frac{1}{d_k} \cdot d_k = 1
$$
